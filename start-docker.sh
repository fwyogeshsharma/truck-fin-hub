...
echo ""
echo "🔄 Running migration 029: Add reconciliation table..."
if [ -f "src/db/migrations/029_add_reconciliation_table.sql" ]; then
  docker exec -i logifin-postgres psql -U "$DB_USER" -d "$DB_NAME" < src/db/migrations/029_add_reconciliation_table.sql
  if [ $? -eq 0 ]; then
    echo "✅ Migration 029 completed successfully!"
  else
    echo "❌ Migration 029 failed!"
    exit 1
  fi
else
...
echo ""
echo "🔄 Running migration 031: Add consignee_sender to contracts..."
if [ -f "src/db/migrations/031_add_consignee_sender_to_contracts.sql" ]; then
  docker exec -i logifin-postgres psql -U "$DB_USER" -d "$DB_NAME" < src/db/migrations/031_add_consignee_sender_to_contracts.sql
  if [ $? -eq 0 ]; then
    echo "✅ Migration 031 completed successfully!"
  else
    echo "❌ Migration 031 failed!"
    exit 1
  fi
else
...
echo ""
echo "🔄 Running migration 033: Fix selected_trip_ids type..."
if [ -f "src/db/migrations/033_fix_selected_trip_ids_type.sql" ]; then
  docker exec -i logifin-postgres psql -U "$DB_USER" -d "$DB_NAME" < src/db/migrations/033_fix_selected_trip_ids_type.sql
  if [ $? -eq 0 ]; then
  ...
  echo "⚠️  Migration file not found: src/db/migrations/033_fix_selected_trip_ids_type.sql"
fi
