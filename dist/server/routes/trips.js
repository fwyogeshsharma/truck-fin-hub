...
import { getAllTrips, getTrip, getTripsByLoadOwner, getTripsByLender, getTripsByStatus, createTrip, updateTrip, addBid, uploadDocument, getTripBids, deleteTrip, bulkCreateTrips, } from '../../src/db/queries/trips.ts';
...
router.get('/', async (req, res) => {
    try {
        ...
        let trips;
        if (status) {
            trips = await getTripsByStatus(status);
        }
        else if (loadOwnerId) {
            trips = await getTripsByLoadOwner(loadOwnerId);
        }
        else if (lenderId) {
            trips = await getTripsByLender(lenderId);
        }
        else {
