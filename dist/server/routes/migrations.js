...
router.post('/add-supported-companies', async (req, res) => {
    try {
        ...
        for (const [key, company] of Object.entries(companyDatabase)) {
            try {
                ...
                const existing = await db.query('SELECT id FROM users WHERE company ILIKE $1 OR name ILIKE $1', [company.name]);
...
router.post('/add-user-approval-columns', async (req, res) => {
    try {
        const db = await getDatabase();
...
router.get('/check-user-columns', async (req, res) => {
    try {
        const db = await getDatabase();
...
router.post('/set-company-admin', async (req, res) => {
    try {
       ...
       WHERE id = $3
       RETURNING id, name, email, company, company_id, is_admin`, [companyId, company.name, userId]);
