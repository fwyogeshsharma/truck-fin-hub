...
import { getWallet, updateWallet, addToBalance, deductFromBalance, moveToEscrow, moveFromEscrowToInvested, returnInvestment, } from '../../src/db/queries/wallets.ts';
...
router.post('/:userId/escrow', async (req, res) => {
    try {
        const { amount } = req.body;
        if (!amount || amount <= 0) {
            return res.status(400).json({ error: 'Valid amount required' });
...
router.post('/:userId/return', async (req, res) => {
    try {
