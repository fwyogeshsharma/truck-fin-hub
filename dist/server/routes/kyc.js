...
import { getUserKyc, getKycById, getAllKyc, getKycByStatus, createOrUpdateUserKyc, updateUserKyc, submitKycForReview, approveKyc, rejectKyc, hasApprovedKyc, getPendingKycCount, getUnderReviewKycCount, } from '../../src/db/queries/userKyc.ts';
const router = Router();
...
router.get('/', (req, res) => {
    try {
        ...
        let kycRecords;
        if (status) {
            kycRecords = getKycByStatus(status);
        }
        else {
...
router.post('/user/:userId/submit', (req, res) => {
    try {
...
router.post('/user/:userId/approve', (req, res) => {
    try {
...
router.post('/user/:userId/reject', (req, res) => {
    try {
