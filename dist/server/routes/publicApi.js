...
const validateTripData = (tripData) => {
    const errors = [];
    // Required fields
    if (!tripData.load_owner_id)
        errors.push('load_owner_id is required');
    if (!tripData.load_owner_name)
        errors.push('load_owner_name is required');
    if (!tripData.origin)
        errors.push('origin is required');
    if (!tripData.destination)
        errors.push('destination is required');
    if (!tripData.load_type)
        errors.push('load_type is required');
    // Amount validation
 ...
 */
router.post('/shippers/trips', validateBasicAuth, validateApiToken, async (req, res) => {
    try {
        const body = req.body;
