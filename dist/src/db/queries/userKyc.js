 ...
 */
export const getKycByStatus = async (status) => {
    const db = await getDatabase();
 ...
 */
export const createOrUpdateUserKyc = async (input) => {
    const db = await getDatabase();
    // Check if KYC exists
    const existing = await getUserKyc(input.user_id);
    if (existing) {
        // Update existing
        return (await updateUserKyc(input.user_id, input));
    }
    // Create new
 ...
 * Update KYC
 */
export const updateUserKyc = async (userId, input) => {
    const db = await getDatabase();
 ...
 */
export const submitKycForReview = async (userId) => {
    const db = await getDatabase();
    await db.query(`
 ...
 * Approve KYC
 */
export const approveKyc = async (userId, verifiedBy) => {
    const db = await getDatabase();
    await db.query(`
    UPDATE user_kyc
 ...
 * Reject KYC
 */
export const rejectKyc = async (userId, verifiedBy, rejectionReason) => {
    const db = await getDatabase();
    await db.query(`
    UPDATE user_kyc
 ...
 */
export const hasApprovedKyc = async (userId) => {
    const kyc = await getUserKyc(userId);
 ...
 */
export const getUnderReviewKycCount = async () => {
    const db = await getDatabase();
