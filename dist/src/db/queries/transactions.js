 ...
 */
export const getTransactionsByUser = async (userId, limit) => {
    const db = getDatabase();
    const query = limit
...
export const getTransactionsByUserAndType = async (userId, type) => {
    const db = getDatabase();
    const result = await db.query('SELECT * FROM transactions WHERE user_id = $1 AND type = $2 ORDER BY timestamp DESC', [userId, type]);
    return result.rows.map(txn => ({
        ...txn,
        amount: Number(txn.amount),
        balance_after: Number(txn.balance_after),
 ...
 */
export const getTransactionsByUserAndCategory = async (userId, category) => {
    const db = getDatabase();
    const result = await db.query('SELECT * FROM transactions WHERE user_id = $1 AND category = $2 ORDER BY timestamp DESC', [userId, category]);
    return result.rows.map(txn => ({
        ...txn,
        amount: Number(txn.amount),
        balance_after: Number(txn.balance_after),
...
export const createTransaction = async (input) => {
    ...
    const result = await db.query(`INSERT INTO transactions (
 ...
 */
export const getTotalCreditedByUser = async (userId) => {
    const db = getDatabase();
    const result = await db.query(`SELECT SUM(amount) as total FROM transactions
 ...
 */
export const getTotalDebitedByUser = async (userId) => {
    const db = getDatabase();
    const result = await db.query(`SELECT SUM(amount) as total FROM transactions
